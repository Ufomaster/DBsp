...
GO
--$Create:     Oleynik Yuiriy$    		$Create date:   23.01.2012$
--$Modify:     Zapadinskiy Anatoliy$    $Modify date:   03.07.2017$
--$Version:    1.00$   $Description: смотрим PropHistoryValueID, т.е привязка расчета только к НЕ ручным значениям.$
CREATE PROCEDURE [dbo].[sp_ProductionCardCustomizeMaterials_AutoCreate]
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @RecreateType = 1
        BEGIN
            INSERT INTO ProductionCardCustomizeMaterials(ProductionCardCustomizeID, TmcID, [Norma], OriginalNorma, PlanNorma)
            SELECT @ID, TmcID, SUM(ISNULL(Norma,0)), SUM(ISNULL(Norma,0)), SUM(ISNULL(Norma,0))
            FROM #Res
            WHERE Norma <> 0
            ...
            INSERT INTO ProductionCardCustomizeMaterials(ProductionCardCustomizeID, TmcID, [Norma], OriginalNorma, PlanNorma)
            SELECT @ID, pccd.TmcID, SUM(ISNULL(pccd.Norma*@CardCountInvoice,0)), SUM(ISNULL(pccd.Norma*@CardCountInvoice,0)), SUM(ISNULL(pccd.Norma*@CardCountInvoice,0))
        ...
        BEGIN
            ...
            --вставляем новые
            INSERT INTO ProductionCardCustomizeMaterials(ProductionCardCustomizeID, TmcID, [Norma], OriginalNorma, PlanNorma)
            SELECT @ID, a.TmcID, SUM(ISNULL(a.Norma,0)), SUM(ISNULL(a.Norma,0)), SUM(ISNULL(a.Norma,0))
            FROM #Res a
            ...
            INNER JOIN (SELECT SUM(ISNULL(a.Norma,0)) AS Summa, a.TmcID
                        ...
                        WHERE a.Norma <> 0 AND m.ID IS NULL AND m.ProductionCardCustomizeID = @ID
