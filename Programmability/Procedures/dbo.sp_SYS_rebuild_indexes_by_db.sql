...
*/
create PROCEDURE [dbo].[sp_SYS_rebuild_indexes_by_db]
  @DBName NVARCHAR(128)        -- Name of the db  
, @IncludedTables NVARCHAR(MAX) = '' -- Comma delimited list of tables (DB.schema.Table) to include in processing
        ...
        SELECT a.[object_id], a.[index_id], a.[partition_number], CAST(a.[avg_fragmentation_in_percent] AS TINYINT) AS [AvgFragPercent], a.[index_type_desc]
            ...
            LEFT JOIN sys.schemas s on t.schema_id = s.schema_id
                WHERE a.[avg_fragmentation_in_percent] >= @ReorgLimit AND a.[page_count] >= @PageLimit 
                      ...
                      AND (a.[object_id] IN (SELECT ISNULL(OBJECT_ID(p.ID),1) FROM dbo.fn_StringToSTable(@IncludedTables) AS p) OR @IncludedTables = '')
