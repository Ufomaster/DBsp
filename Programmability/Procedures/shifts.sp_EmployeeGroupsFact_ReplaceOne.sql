...
CREATE PROCEDURE [shifts].[sp_EmployeeGroupsFact_ReplaceOne]
    @EmployeeID int,
    @ShiftID int,
    @WorkPlaceID int,
    @OldWorkPlaceID int
...
BEGIN
    ...
    BEGIN TRAN
    BEGIN TRY
        EXEC shifts.sp_EmployeeGroupsFact_Delete @EmployeeID, @ShiftID, @OldWorkPlaceID
        ...
        IF NOT EXISTS(SELECT g.ID
                      ...
                      WHERE g.ShiftID = @ShiftID AND g.WorkPlaceID = @WorkPlaceID AND ISNULL(g.IsDeleted, 0) = 0)
        BEGIN
