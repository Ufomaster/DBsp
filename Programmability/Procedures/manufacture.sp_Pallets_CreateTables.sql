...
/*$Modify:     Yuriy Oleynik$    $Modify date:   28.09.2018$*/
...
BEGIN
    ...
    DECLARE @TableName varchar(50), @TableNameD varchar(50), @TableBoxName varchar(50)

    /*Create main table*/
    SET @TableName = 'Pallets_' + Convert(varchar(30), @JobStageID)
    IF NOT EXISTS(SELECT *
                  FROM information_schema.tables t
                  WHERE t.TABLE_SCHEMA = 'StorageData' AND t.TABLE_NAME = @TableName)
    BEGIN
        EXEC ('CREATE TABLE [StorageData].['+@TableName+'] (
    ...
    IF NOT EXISTS (SELECT * 
                   FROM information_schema.tables t 
                   WHERE t.TABLE_SCHEMA = 'StorageData' AND t.TABLE_NAME = @TableNameD)
    BEGIN
        EXEC ('CREATE TABLE [StorageData].['+@TableNameD+'] (
                  [ID] int IDENTITY(1, 1) NOT NULL,
