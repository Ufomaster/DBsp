...
/*$Modify:     Oleynik Yuriy$    $Modify date:   01.10.2018*/
...
/*    1 can view. 0 no view*/
    SELECT @CanLaunch = CASE WHEN MAX(ISNULL(uro1.RightValue, 1)) = 3 THEN 1 ELSE 0 END
    FROM RoleUsers ru
    ...
    BEGIN            
        RAISERROR ('По данному участку уже запущенно сменное задание. Запуск двух одновременно запрещен', 16, 1)
        RETURN
    END
    ELSE        
